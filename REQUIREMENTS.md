...
| **Add a sphere…** | Load an image from this computer. It joins the list and is used at once. |
...
in as blobs — that file is the record of what is in each one. A loaded sphere
lasts for the session: the saved defaults carry which sphere was chosen, not the
image, because they go to one small cloud document and an image does not fit in
it.
